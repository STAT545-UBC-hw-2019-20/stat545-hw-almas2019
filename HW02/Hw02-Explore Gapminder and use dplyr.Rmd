...
```
##Plot 2: Bar Graph Showing Number of Cylinders and Numbers of Cars for each transmission type
```{r}
mtcars %>%
  mutate(cyl=as.factor(cyl)) %>%
  mutate(automatic_transmiss=if_else(am<=0,"Yes","No")) %>%
  ggplot(aes(x=cyl,group=automatic_transmiss))+
  geom_bar(aes(fill=automatic_transmiss))+
